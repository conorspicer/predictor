...
picks = pd.DataFrame(list(Pick.objects.filter(user__username="magnusmartinsen").values()))
fixtures = pd.DataFrame(list(Fixture.objects.all().values()))
picks.set_index('fixture_id').join(fixtures.set_index('id')).to_csv('magnus.csv')
